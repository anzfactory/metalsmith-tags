...
function plugin(opts) {
  ...
  return function(files, metalsmith, done) {
    ...
    for (var fileName in files) {
      ...
      if (tagsData) {
        ...
        tagsData.forEach(function(rawTag) {
          ...
          var tag = ''
          var slug = ''
          if (typeof rawTag === 'object' && typeof rawTag.name !== 'undefined' && typeof rawTag.slug !== 'undefined') {
            tag = String(rawTag.name).trim();
            slug = String(rawTag.slug).trim();
          ...
          data[opts.handle].push({ name: tag, slug: slug});
    ...
    for (var tag in tagList) {
      ...
      var posts = tagList[tag].map(function(info) {
        return files[info.fileName];
      }).sort(sortBy);
      ...
      var pages = [];
      console.log('->>' + tagList[tag][0].slug)
